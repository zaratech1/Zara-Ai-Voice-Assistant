(() => {
  ...
  function state(name, label) {
    ...
    $('heroStatus').textContent = label || name;
  ...
  async function selectChat(id) {
    try{
      ...
      activeChat=id;$('messages').replaceChildren();$('welcome').hidden=messages.length>0;
      for(const message of messages)renderMessage(message);
      $('currentTitle').textContent=chats.find(chat=>chat.id===id)?.title||'New chat';
      renderChats();$('messageInput').focus();shell.classList.remove('mobile-open');
  ...
  function onEvent(event,data){
    ...
    if(event==='ai_test'){$('aiFeedback').textContent=data.message;$('aiFeedback').dataset.status=data.success?'connected':'offline';return}
    if(event==='ai_status'){$('aiFeedback').textContent=data.status==='connected'?'● Connected':'○ Offline';$('aiFeedback').dataset.status=data.status;return}
    if(event==='wake_status'){const labels={active:'● Wake word active',starting:'◌ Wake word starting',paused:'○ Wake word paused',detected:'● Hey Zara detected',error:'! Wake word unavailable',off:'○ Wake word off'};$('wakeIndicator').textContent=labels[data.status]||labels.off;$('wakeIndicator').dataset.status=data.status;if(data.message&&data.status==='error')notice(data.message);return}
    if(event==='error'||event==='status'){notice(data.message);setBusy(false);return}
    if(event==='command_failed'){notice(data.message);setBusy(false)}
  }
  window.backend_event=onEvent;
  if(window.eel)eel.expose(window.backend_event,'backend_event');

  function openSettings(){fillSettings();$('settingsModal').hidden=false;$('closeSettingsBtn').focus()}
  ...
  async function saveSettings(){
    ...
    try{settings=await api('update_settings',values);document.documentElement.dataset.theme=settings.theme;shell.classList.toggle('collapsed',settings.sidebar_collapsed);$('aiFeedback').textContent='○ Not tested';$('aiFeedback').dataset.status='unknown';$('settingsFeedback').textContent='Saved';setTimeout(()=>$('settingsFeedback').textContent='',2500);return true}catch(error){$('settingsFeedback').textContent=error.message;return false}
  }
  ...
  async function bootstrap(){try{const data=await api('bootstrap');chats=data.chats;settings=data.settings;voices=data.voices;document.documentElement.dataset.theme=settings.theme;shell.classList.toggle('collapsed',settings.sidebar_collapsed);if(!chats.length||(settings.launch_behavior==='new_chat'&&!sessionStorage.getItem('zaraLaunched')))await createChat();else await selectChat(chats[0].id);sessionStorage.setItem('zaraLaunched','1');renderChats();fillSettings()}catch(error){notice(error.message);$('topStatus').textContent='Disconnected'}}
  ...
  $('collapseBtn').onclick=()=>{shell.classList.add('collapsed');api('update_settings',{sidebar_collapsed:true}).then(data=>settings=data).catch(error=>notice(error.message))};
  $('menuBtn').onclick=()=>{if(innerWidth<=760)shell.classList.toggle('mobile-open');else{shell.classList.remove('collapsed');api('update_settings',{sidebar_collapsed:false}).then(data=>settings=data).catch(error=>notice(error.message))}};
  ...
  document.querySelectorAll('.suggestion').forEach(button=>button.onclick=()=>submit(button.dataset.prompt));
  document.onkeydown=event=>{if(event.key==='Escape'){if(!$('confirmModal').hidden)confirmWhatsApp(false);else closeSettings();return}if(event.ctrlKey&&event.key.toLowerCase()==='n'){event.preventDefault();createChat()}};
  bootstrap();
})();
